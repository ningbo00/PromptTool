...
from shared.ui_kit import (
    ...
    ACCENT_CYAN, BG_HOVER, DARK_TEXT,
)
...
class AISettingsDialog(tk.Toplevel):
    ...
    def _build_ui(self):
        provider_box = tk.Frame(self, bg=BG_BASE)
        provider_box.pack(fill=tk.X, padx=20, pady=(14, 8))
        tk.Label(provider_box, text="当前服务商:", bg=BG_BASE, fg=FG_MUTED,
                 font=("微软雅黑", 9)).pack(anchor="w")

        provider_grid = tk.Frame(provider_box, bg=BG_BASE)
        ...
        tk.Entry(form, textvariable=self._key_var, bg=BG_CARD, fg=FG_PRIMARY,
                 ...
                 show="*", width=52).grid(row=0, column=1, sticky="ew", padx=(8, 0), ipady=4)
        ...
        self._note_lbl = tk.Label(self, text="", bg=BG_BASE, fg=FG_DIM,
                                  font=("微软雅黑", 8), wraplength=590, justify=tk.LEFT)
