...
class InstructionPanel:
    ...
    def _build_options(self, presets, length_var):
        ...
        for label in ("简短", "中等", "详细"):
            tk.Radiobutton(row, text=label, variable=length_var, value=label,
                           bg=BG_ELEVATED, fg=FG_PRIMARY, activebackground=BG_ELEVATED,
                           selectcolor=BG_CARD, font=(FONT_FAMILY, 9)
                           ).pack(side=tk.LEFT, padx=(4, 0))

        self.history_button = tk.Menubutton(
            row, text="🕐 历史 ▾", bg=BG_CARD, fg=ACCENT_BLUE,
            relief=tk.FLAT, font=(FONT_FAMILY, 9, "bold"), padx=8, pady=3,
    ...
    def _build_custom_input(self):
        row = tk.Frame(self.frame, bg=BG_ELEVATED)
        row.pack(fill=tk.X)
        tk.Label(row, text="自定义指令:", bg=BG_ELEVATED, fg=FG_MUTED,
                 font=(FONT_FAMILY, 9)).pack(side=tk.LEFT)
...
class ResultPanel:
    ...
    def build_header(parent, diff_var, on_toggle_diff, on_copy_result):
        ...
        tk.Label(header, text="Result Matrix", bg=BG_BASE, fg=ACCENT_GREEN,
                 font=(FONT_FAMILY, 9, "bold")).pack(side=tk.LEFT)
...
class InsightsPanel:
    ...
    def build_keywords(frame, keywords, on_copy, on_close):
        ...
        for row_index in range(rows_needed):
            ...
            for keyword in keywords[row_index * 8: (row_index + 1) * 8]:
                tk.Button(
                    row, text=keyword, bg=BG_CARD, fg=ACCENT_BLUE,
    ...
    def build_score(frame, on_improve, on_close):
        ...
        improve_button = tk.Button(
            ...
            bg=BG_CARD, fg=ACCENT_ORANGE, relief=tk.FLAT,
            font=(FONT_FAMILY, 8, "bold"), padx=8, pady=1,
            cursor="hand2", activebackground=BG_HOVER, state=tk.DISABLED,
        ...
        tk.Button(score_header, text="✕", command=on_close,
                  ...
                  font=(FONT_FAMILY, 8), padx=4, pady=0,
    ...
    def build_compliance(frame, on_fix, on_close):
        ...
        fix_button = tk.Button(
            ...
            bg=BG_CARD, fg=ACCENT_ORANGE, relief=tk.FLAT,
            font=(FONT_FAMILY, 8, "bold"), padx=10, pady=2,
            cursor="hand2", activebackground=BG_HOVER, state=tk.DISABLED,
            highlightbackground=ACCENT_ORANGE, highlightthickness=1,
        )
    ...
    def build_negative_recommendations(frame, groups, on_copy, on_copy_all, on_close):
        ...
        if all_words:
            tk.Button(header, text="📋 全部复制", command=lambda: on_copy_all(all_words),
                      ...
                      font=(FONT_FAMILY, 8, "bold"), padx=8, pady=1,
                      cursor="hand2", activebackground=BG_HOVER,
                      highlightbackground=ACCENT_RED, highlightthickness=1).pack(side=tk.RIGHT, padx=(0, 4))
